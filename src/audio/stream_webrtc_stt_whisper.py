...
def transcribe_audio(audio_segment: AudioSegment, debug: bool = False) -> str:
    """
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmpfile:
        ...
        breakpoint()
        answer = model.transcribe(samples)["text"]
...
def handle_silence(
    sound_chunk, silence_frames, silence_frames_threshold, text_output
):
    """
    ...
    if silence_frames >= silence_frames_threshold:
        if len(sound_chunk) > 0:
            text = transcribe_audio(sound_chunk)
...
def app_sst(
    ...
    while True:
        if webrtc_ctx.audio_receiver:
            ...
            if len(sound_chunk) > 0:
                text = transcribe_audio(sound_chunk.raw_data)
                text_output.write(text)
            break
